...
public class Mainframe extends JFrame {
	...
	JButton btnCopyResp = null;

	/** The chk del assoc. */
	JCheckBox chkDelAssoc = new JCheckBox(
	...
	public Mainframe(SQliteOps _Sqliteops, Crypto _Crypto) {
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (l_sqliteops.deleteRecord(dbRec) < 0) {
					...
					if (chkDelAssoc.isSelected()) {
						DefaultTableModel model = (DefaultTableModel) jtabCreds
								.getModel();
						model.setRowCount(0);
					}
					clearFields();
		...
		// / Begin section for credentials challenges/responses - text fields
		// and buttons
		...
		btnCredAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (FormValidation.verifyCredData(jtxtChlng.getText()
						.toString(), jtxtRsp.getText().toString()) < 0) {
					JOptionPane.showMessageDialog(null,
							"Challenge and Response are required");
				} else {
					dbRec = new DbRecord();
					dbRec.setType(Pwdtypes.S_CREDS_TYPE);
					dbRec.setClientId(iClientId);
					dbRec.setChallenge(jtxtChlng.getText().toString());
					dbRec.setResponse(l_crypto.encrypt(jtxtRsp.getText()
							.toString()));
					...
					if (!jcbTrack.getSelectedItem().toString().equals("0")) {
						...
						SimpleDateFormat sdf = new SimpleDateFormat(
								"MM/dd/yyyy");
						int iDaysToAdd = Integer.parseInt(jcbTrack
				...
				if (dbRec.getType().equals(Pwdtypes.S_CREDS_TYPE)
					...
					if (!sChallenge.equals(jtxtChlng.getText())) {
						sbLogMsg.append("Application [" + jtxtApp.getText()
								+ "], Challenge modified, old [" + sChallenge
								+ "], new [" + jtxtChlng.getText() + "]");
					}
					...
					if (sbLogMsg.toString().length() > 0) {
						...
						if (l_sqliteops.insertRecord(dbRecLog) < 0) {
							JOptionPane.showMessageDialog(
									null,
									"Insert log record failed ["
											+ dbRecLog.getResult() + "]");
						}
					}

					if (!jcbTrack.getSelectedItem().toString().equals("0")) {
						iDaysToAdd = Integer.parseInt(jcbTrack
								.getSelectedItem().toString());
					...
					if (l_sqliteops.updateRecord(dbRec) < 0) {
						...
						jtabCreds.setValueAt(jtxtChlng.getText().toString(),
								iRow, 1);
						jtabCreds.setValueAt(
								l_crypto.encrypt(jtxtRsp.getText().toString()),
								iRow, 2);
						jtabCreds.setValueAt(jcbTrack.getSelectedItem()
								.toString(), iRow, 3);
		...
		btnCredDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (l_sqliteops.deleteRecord(dbRec) < 0) {
					JOptionPane.showMessageDialog(
							null,
							"Delete credential record failed ["
									+ dbRec.getResult() + "]");
				} else {
					DefaultTableModel jtabModel = (DefaultTableModel) jtabCreds
							.getModel();
		...
		// / End section for credentials challenges/responses - text fields and
		// buttons
		...
		btnShowAssoc = new JButton(
				"Display Associated Challenges/Responses in new window");

		// / Display the challenges/responses associated to the application in a
		// popup window.
		...
		btnShowAssoc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				sDisplay += "\n";
				DefaultTableModel jTmpModel = (DefaultTableModel) jtabCreds
						.getModel();
				for (int i = 0; i < jTmpModel.getRowCount(); i++) {
					sDisplay += "Q. "
							+ jTmpModel.getValueAt(i, 1).toString()
							+ "  A. "
							+ l_crypto.decrypt(jTmpModel.getValueAt(i, 2)
									.toString()) + "\n";
				}
		...
		// / Values for expiration days are hardcoded, may want to move to a
		// table for metadata
		jcbTrack.addItem("0");
		jcbTrack.addItem("30");
		jcbTrack.addItem("45");
		...
		btnCopyResp = new JButton("Copy Data");
		btnCopyResp.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (jtxtRsp.getText().toString().length() > 0) {
					Toolkit.getDefaultToolkit()
							.getSystemClipboard()
							.setContents(
									new StringSelection(jtxtRsp.getText()
											.toString()), null);
				}
			}
		});
												...
												gl_contentPane
														...
														.addGroup(
																gl_contentPane
																		...
																		.addComponent(
																				btnShowAssoc)))
										.addGap(18).addComponent(chkDelAssoc)
		...
		jtabCreds.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "ID", "Challenge", "Response", "Exp Days",
						"Expiration Date" }) {
		...
		jtabCreds.setBorder(new MatteBorder(1, 1, 1, 1, (Color) new Color(0, 0,
				0)));
		...
		contentPane.setFocusTraversalPolicy(new FocusTraversalOnArray(
				new Component[] { jtxtChlng, lblThePasswordSaver, jtxtApp,
						jtxtDesc, btnAdd, btnReplace, btnDelete, btnSearch,
						btnClear, jtxtRsp, btnCredAdd, btnCredReplace,
						btnCredDelete, btnCredClear, scrollPane, jtabApps,
						lblUrlapplication, lblDescription, chkDelAssoc,
						lblChallenge, lblResponse, scrollPane_1, jtabCreds }));
		setFocusTraversalPolicy(new FocusTraversalOnArray(new Component[] {
				menuBar, jtxtApp, jtxtDesc, btnAdd, btnReplace, btnDelete,
				btnSearch, btnClear, jtxtChlng, jtxtRsp, btnCredAdd,
				btnCredReplace, btnCredDelete, btnCredClear, contentPane,
				mnFile, mntmExit, lblThePasswordSaver, scrollPane, jtabApps,
				lblUrlapplication, lblDescription, chkDelAssoc, lblChallenge,
				lblResponse, scrollPane_1, jtabCreds }));
		...
		rowCred.addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (!lsmData.isSelectionEmpty()) {
					int iRow = lsmData.getMinSelectionIndex();
					iCredId = Integer.parseInt(jtabCreds.getValueAt(iRow, 0)
							.toString());
					jtxtChlng.setText(jtabCreds.getValueAt(iRow, 1).toString());
					jtxtRsp.setText(l_crypto.decrypt(jtabCreds.getValueAt(iRow,
							2).toString()));
					jcbTrack.setSelectedItem(jtabCreds.getValueAt(iRow, 3)
							.toString());
					if (null == jtabCreds.getValueAt(iRow, 4)) {
						sDateModified = "";
					} else {
						sDateModified = jtabCreds.getValueAt(iRow, 4)
								.toString();
					}
	...
	public void addToCredsTable() {
		...
		jtabModel.addRow(new Object[] { dbRec.getCredId(),
				dbRec.getChallenge(), dbRec.getResponse(), dbRec.getTrack(),
				dbRec.getModifyDate() });
		clearCredsFields();
		enableCredsButtons();
	}
	...
	/**
	 * disableCredsButtons - Disable buttons associated with credentials
	 * manipulation.
	 */
	...
	public void loadTable(DbRecord _dbRec) {
		...
		try {
			...
			if (_dbRec.getType().equals(Pwdtypes.S_CLIENT_TYPE)) {
				...
				jtabCreds.setModel(new DefaultTableModel(new Object[][] {},
						new String[] { "ID", "Challenge", "Response",
								"Exp Days", "Expiration Date" }));
